...
cls
	...
	if %option0%==4 goto relacionamento

:atualizadormanual
	cls
	set "params=%*"
	cd /d "%~dp0" && ( if exist "%temp%\getadmin.vbs" del "%temp%\getadmin.vbs" ) && fsutil dirty query %systemdrive% 1>nul 2>nul || (  echo Set UAC = CreateObject^("Shell.Application"^) : UAC.ShellExecute "cmd.exe", "/k cd ""%~sdp0"" && %~s0 %params%", "", "runas", 1 >> "%temp%\getadmin.vbs" && "%temp%\getadmin.vbs" && exit /B )
	powershell -Command "Start-BitsTransfer -Source 'https://fenoxteccombr-my.sharepoint.com/:u:/g/personal/alan_silva_fenoxtec_com_br/IQAKy4fViNdWR6nCqcDeRkxGAbXtT3uVdVNQH8QkmOJCn8U?e=PyABzc&download=1' -Destination '%temp%\ATUALIZADOR_ESTADOS.bat'" >nul 2>&1 && start "" "%temp%\ATUALIZADOR_ESTADOS.bat" && exit
	Exit
	goto :fim
