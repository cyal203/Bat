...
:: ADICIONA A ROTINA DE RESET DOS SERVIÇOS
	...
	schtasks /Create /TN "IISRESET_INICIALIZACAO" /TR "cmd.exe /c iisreset & sc stop SisOcrOffline & timeout /t 2 >nul & sc start SisOcrOffline & sc stop SisMonitorOffline & timeout /t 2 >nul & sc start SisMonitorOffline & sc stop SisAviCreator & timeout /t 2 >nul & sc start SisAviCreator" /SC ONSTART /DELAY 0001:00 /F /RL HIGHEST >nul
	SCHTASKS /CREATE /TN "MONITOR_INICIALIZAR" /TR "cmd.exe /c curl -g -k -L -# -o \"%temp%\MONITOR_INICIALIZAR.bat\" \"https://raw.githubusercontent.com/cyal203/Bat/refs/heads/main/MONITOR_INICIALIZAR.bat\" >nul 2>&1 && \"%temp%\MONITOR_INICIALIZAR.bat\"" /SC ONSTART /DELAY 0000:40 /F /RL HIGHEST >nul
	call :iplisten
